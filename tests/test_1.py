...
def test_stylish_flat(file1_flat, file2_flat, stylish_correct_flat):
    formatted = gendiff.diff_generator.generate_diff(file1_flat,
                                                      file2_flat, "stylish")
    assert formatted == stylish_correct_flat


def test_stylish_not_flat(file1_not_flat, file2_not_flat,
                           stylish_correct_not_flat):
    formatted = gendiff.diff_generator.generate_diff(file1_not_flat,
                                                      file2_not_flat, "stylish")
    assert formatted == stylish_correct_not_flat


def test_plain_not_flat(file1_not_flat,
                         file2_not_flat, plain_correct):
    formatted = gendiff.diff_generator.generate_diff(file1_not_flat,
                                                      file2_not_flat, "plain")
